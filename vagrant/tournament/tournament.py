...
def deletePlayers(tournament_id=1):
    ...
    (database, cursor) = connect()
    cursor = database.cursor()
    delete = "DELETE FROM players WHERE tournament_id = %s;"
...
def registerTournament(tournament_name):
    ...
    (database, cursor) = connect()
    insert = "INSERT INTO tournaments VALUES (DEFAULT, %s);"
...
def registerPlayer(player_name, tournament_id=1):
    ...
    if tournament_id == 1:
        (database, cursor) = connect()
        query = "SELECT tournament_id FROM tournaments WHERE tournament_id = 1;"
    ...
    (database, cursor) = connect()
    insert = "INSERT INTO players VALUES (%s, DEFAULT, %s);"
...
def playerStandings(tournament_id=1):
    ...
    Here is the scoring system:
        Win/Bye: +3
        Tie: +1
        OMW: + A fraction from 0-1, rounded to 2 decimal.  Stands for Opponent
             Match Wins. The fraction represents total wins / total matches for
             all the opponents of the player in question.
    ...
    query = """
        SELECT player_id, player_name, total_wins, total_matches, total_ties, bye
        FROM totalRecordWithMetadata
        WHERE tournament_id = %s;"""
    cursor.execute(query, (tournament_id,))
    total_record = cursor.fetchall()
    database.close()
    player_standings = []
    # Loop through each player record tuple in total record, append the score to
    # the tuple, and append the tuple to player_standings
...
def calculate_OMW(player_id, total_record):
    ...
    query = """
    SELECT opponent_one FROM matchRegistry
...
def reportMatch(winner, loser, tie=None, bye=None, tournament_id=None):
    ...
    if winner and bye and not (loser or tie):
        ...
        insert = """
            INSERT INTO matchRegistry (tournament_id, match_id, opponent_one)
            VALUES (%s, DEFAULT, %s);"""
        cursor.execute(insert, (tournament_id, winner))
        database.commit()
        # Pull the match ID that was just committed
        query = "SELECT match_id from matchRegistry WHERE opponent_one = %s;"
        cursor.execute(query, (winner,))
    ...
    if winner > loser:
        insert = "INSERT INTO matchRegistry VALUES (%s, DEFAULT, %s, %s);"
...
def swissPairings(tournament_id=1):
    ...
    if number_of_players % 2 == 1:
        ...
        for (place, player_record) in enumerate(reverse_player_standings):
            ...
            # create the swisspairings tuples and return them
            if bye == 0:
                database.close()
